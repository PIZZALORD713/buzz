...
use crate::{verify_nip98_event, AuthError, Nip98ReplayGuard, DEFAULT_REPLAY_TTL_SECS};
...
/// Opaque, fully verified lifecycle authority awaiting its one atomic replay
/// claim.
///
/// Verification performs no replay mutation. The configured caller may use
/// the trusted quota coordinates below, then consume this value with
/// [`Self::claim_replay`] only after quota admission succeeds. This value is
/// intentionally non-cloneable, non-serializable, and has no public
/// constructor.
///
/// ```compile_fail
/// use buzz_auth::VerifiedOperatorLifecycleCandidate;
/// let _forged = VerifiedOperatorLifecycleCandidate {};
/// ```
pub struct VerifiedOperatorLifecycleCandidate {
    grant: VerifiedOperatorLifecycleGrant,
    replay_scope: Box<str>,
    replay_event_ids: Box<[EventId]>,
}

impl VerifiedOperatorLifecycleCandidate {
    /// Server-resolved domain derived from the verified request.
    pub const fn authorization_domain(&self) -> CommunityId {
        self.grant.authorization_domain()
    }

    /// Exact closed lifecycle action derived from the verified request.
    pub const fn action(&self) -> OperatorLifecycleAction {
        self.grant.action()
    }

    /// Stable operation identity derived from the verified request.
    pub const fn operation_id(&self) -> Uuid {
        self.grant.operation_id()
    }

    /// Pseudonymous quota key derived from the configured verified signer.
    pub const fn signer_attribution(&self) -> [u8; 32] {
        self.grant.signer_attribution()
    }

    /// Exclusive expiry of all verified authority contributing to the grant.
    pub const fn expires_at(&self) -> DateTime<Utc> {
        self.grant.expires_at()
    }

    /// Atomically claim every sealed proof ID and yield finalized authority.
    ///
    /// The exclusive expiry is rechecked before replay I/O so quota delay can
    /// never burn already-expired evidence. The underlying replay guard must
    /// implement one all-or-none claim for the complete sealed proof set.
    pub async fn claim_replay(
        ...
        if self.grant.expires_at() <= Utc::now() {
            return Err(OperatorLifecycleGrantError::InvalidCredential);
        }
        claim_replay_set(
            replay_guard,
            self.replay_scope.as_ref(),
            self.replay_event_ids.as_ref(),
        )
        .await?;
        Ok(self.grant)
    }
...
impl fmt::Debug for VerifiedOperatorLifecycleCandidate {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("VerifiedOperatorLifecycleCandidate([REDACTED])")
    }
}
...
pub struct VerifiedProvisionBindingIntent {
    ...
    policy_revision: u64,
    policy_digest: [u8; 32],
    principal: crate::FederatedPrincipal,
...
/// Opaque, fully verified provision intent awaiting its one atomic replay
/// claim.
///
/// Verification performs no replay mutation. This value is intentionally
/// non-cloneable, non-serializable, and has no public constructor.
///
/// ```compile_fail
/// use buzz_auth::VerifiedProvisionBindingCandidate;
/// let _forged = VerifiedProvisionBindingCandidate {};
/// ```
pub struct VerifiedProvisionBindingCandidate {
    intent: VerifiedProvisionBindingIntent,
    replay_scope: Box<str>,
    replay_event_ids: Box<[EventId]>,
}

impl VerifiedProvisionBindingCandidate {
    /// Server-resolved domain derived from the verified request.
    pub const fn authorization_domain(&self) -> CommunityId {
        self.intent.authorization_domain()
    }
    ...
    /// Pseudonymous quota key derived from the configured verified signer.
    pub const fn signer_attribution(&self) -> [u8; 32] {
        self.intent.signer_attribution()
    }

    /// Exclusive expiry of all verified authority contributing to the intent.
    pub const fn expires_at(&self) -> DateTime<Utc> {
        self.intent.expires_at()
    }

    /// Atomically claim every sealed proof ID and yield the provision intent.
    pub async fn claim_replay(
        self,
        replay_guard: &dyn Nip98ReplayGuard,
    ) -> Result<VerifiedProvisionBindingIntent, OperatorLifecycleGrantError> {
        if self.intent.expires_at() <= Utc::now() {
            return Err(OperatorLifecycleGrantError::InvalidCredential);
        }
        claim_replay_set(
            replay_guard,
...
impl fmt::Debug for VerifiedProvisionBindingCandidate {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("VerifiedProvisionBindingCandidate([REDACTED])")
    }
}
...
impl OperatorLifecycleVerifier {
    ...
    /// Verify exact NIP-98 authority without claiming replay evidence.
    ///
    /// The returned candidate carries only verifier-derived quota coordinates.
    /// Callers must apply actor/domain quota before consuming it with
    /// [`VerifiedOperatorLifecycleCandidate::claim_replay`].
    ...
    pub async fn verify_candidate(
        ...
        replacement_event_author_pubkey: Option<[u8; 32]>,
        replacement_expires_at: Option<DateTime<Utc>>,
    ) -> Result<VerifiedOperatorLifecycleCandidate, OperatorLifecycleGrantError> {
        ...
        let grant = VerifiedOperatorLifecycleGrant {
            authorization_domain,
            action,
            operation_id,
            intent_digest,
        ...
        Ok(VerifiedOperatorLifecycleCandidate {
            grant,
            replay_scope: replay_scope.into_boxed_str(),
            replay_event_ids: replay_event_ids.into_boxed_slice(),
        })
    }

    /// Verify an exact binding provision body without claiming replay evidence.
    ///
    /// Callers must apply actor/domain quota before consuming the candidate
    /// with [`VerifiedProvisionBindingCandidate::claim_replay`].
    pub async fn verify_provision_candidate(
        &self,
        authorization_event_json: &str,
        independent_approval_event_json: Option<&str>,
        expected_url: &str,
        request_body: &[u8],
        authorization_domain: CommunityId,
    ) -> Result<VerifiedProvisionBindingCandidate, OperatorLifecycleGrantError> {
        if authorization_domain.as_uuid().is_nil() {
            return Err(OperatorLifecycleGrantError::InvalidRequest);
        }
        ...
        replay_event_ids.push(proof.signer_event_id);
        replay_event_ids.extend(proof.approval_event_id);
        let intent = VerifiedProvisionBindingIntent {
            authorization_domain,
        ...
        Ok(VerifiedProvisionBindingCandidate {
            intent,
            replay_scope: replay_scope.into_boxed_str(),
            replay_event_ids: replay_event_ids.into_boxed_slice(),
        })
    ...
    #[cfg(test)]
    async fn verify_provision_binding(
        &self,
        authorization_event_json: &str,
        independent_approval_event_json: Option<&str>,
        expected_url: &str,
        ...
        self.verify_provision_candidate(
            authorization_event_json,
            independent_approval_event_json,
            expected_url,
            request_body,
    ...
    async fn verify_replacement_proof(
        ...
        Ok(Some(VerifiedReplacementPair {
            ...
            issued_at: DateTime::<Utc>::from_timestamp(
...
fn parse_provision_coordinates(
    ...
    let principal = crate::FederatedPrincipal::from_verified_parts(wire.issuer, wire.subject)
...
mod tests {
    ...
    impl Nip98ReplayGuard for ReplaySet {
        fn try_mark_in_scope<'a>(
            ...
            Box::pin(async move {
                Ok(self
                    ...
                    .lock()
                    .expect("replay lock")
                    .insert((scope.to_owned(), event_id.to_bytes())))
            })
        }
    ...
    impl ReplaySet {
        fn len(&self) -> usize {
            self.seen.lock().expect("replay lock").len()
        }
    ...
    fn provision_body(domain: CommunityId, operation_id: Uuid, selected: &Keys) -> Vec<u8> {
    ...
    #[tokio::test]
    async fn lifecycle_candidates_defer_one_two_and_three_proof_claims_until_quota_admission() {
        ...
        let proof = signed(&signer, &url, &body, intent_digest);
        let replay = ReplaySet::default();
        let dropped = verifier
            .verify_candidate(
                &proof,
                None,
                None,
            ...
            .await
            .expect("verify one-proof candidate");
        assert_eq!(dropped.authorization_domain(), domain);
        assert_eq!(dropped.operation_id(), operation_id);
        assert_eq!(dropped.action(), OperatorLifecycleAction::Retire);
        assert_eq!(
            dropped.signer_attribution(),
            operator_attribution(signer.public_key())
        );
        assert!(dropped.expires_at() > Utc::now());
        assert_eq!(
            format!("{dropped:?}"),
            "VerifiedOperatorLifecycleCandidate([REDACTED])"
        );
        assert_eq!(replay.len(), 0);
        assert_eq!(replay.batch_calls(), 0);
        drop(dropped); // Quota denial consumes no replay evidence.
        let grant = verifier
            .verify_candidate(
                &proof,
                None,
            ...
            .await
            .expect("reverify after quota denial")
            .claim_replay(&replay)
            .await
            .expect("claim one-proof candidate");
        assert_eq!(
            grant.signer_attribution(),
            operator_attribution(signer.public_key())
        );
        assert_eq!(replay.len(), 1);
        assert_eq!(replay.batch_calls(), 1);
        ...
        .expect("configure independent verifier");
        let operation_id = Uuid::new_v4();
        let intent_digest = [42; 32];
        let body = lifecycle_body(operation_id, intent_digest, None);
        let url = format!(
        ...
        let candidate = verifier
            .verify_candidate(
                ...
                Some(&signed(&approver, &url, &body, intent_digest)),
                None,
                &url,
                &body,
                domain,
            ...
            .await
            .expect("verify two-proof candidate");
        assert_eq!(replay.len(), 0);
        assert_eq!(replay.batch_calls(), 0);
        candidate
            .claim_replay(&replay)
            .await
            .expect("claim two-proof candidate");
        assert_eq!(replay.len(), 2);
        assert_eq!(replay.batch_calls(), 1);

        let replacement = Keys::generate();
        let operation_id = Uuid::new_v4();
        let intent_digest = [43; 32];
        ...
        let candidate = verifier
            ...
            .await
            .expect("verify three-proof candidate");
        assert_eq!(replay.len(), 0);
        assert_eq!(replay.batch_calls(), 0);
        candidate
            .claim_replay(&replay)
            .await
            .expect("claim three-proof candidate");
        assert_eq!(replay.len(), 3);
        assert_eq!(replay.batch_calls(), 1);
    }

    #[tokio::test]
    async fn provision_candidates_defer_one_and_two_proof_claims_until_quota_admission() {
        let domain = CommunityId::from_uuid(Uuid::new_v4());
        let operation_id = Uuid::new_v4();
        let signer = Keys::generate();
        let selected = Keys::generate();
        let body = provision_body(domain, operation_id, &selected);
        let url = "https://operator.example/operator/identity-bindings/provision";
        let proof = signed(&signer, url, &body, [1; 32]);
        let verifier =
            OperatorLifecycleVerifier::new(vec![signer.public_key()]).expect("configure verifier");
        let replay = ReplaySet::default();
        let dropped = verifier
            .verify_provision_candidate(&proof, None, url, &body, domain)
            .await
            .expect("verify one-proof provision candidate");
        assert_eq!(dropped.authorization_domain(), domain);
        assert_eq!(dropped.operation_id(), operation_id);
        assert_eq!(
            dropped.signer_attribution(),
        ...
        assert_eq!(
            format!("{dropped:?}"),
            "VerifiedProvisionBindingCandidate([REDACTED])"
        );
        assert_eq!(replay.len(), 0);
        assert_eq!(replay.batch_calls(), 0);
        drop(dropped); // Quota denial consumes no replay evidence.
        verifier
            .verify_provision_candidate(&proof, None, url, &body, domain)
            .await
            .expect("reverify provision after quota denial")
            .claim_replay(&replay)
            .await
            .expect("claim one-proof provision candidate");
        assert_eq!(replay.len(), 1);
        assert_eq!(replay.batch_calls(), 1);
        ...
        let candidate = verifier
            .verify_provision_candidate(
                ...
                Some(&signed(&approver, url, &body, [1; 32])),
                url,
                &body,
                domain,
            )
            .await
            .expect("verify two-proof provision candidate");
        assert_eq!(replay.len(), 0);
        assert_eq!(replay.batch_calls(), 0);
        candidate
            .claim_replay(&replay)
            .await
            .expect("claim two-proof provision candidate");
        assert_eq!(replay.len(), 2);
        assert_eq!(replay.batch_calls(), 1);
    }

    #[tokio::test]
    async fn candidate_claims_are_atomic_one_winner_and_expiry_fails_before_io() {
        let signer = Keys::generate();
        let verifier =
            OperatorLifecycleVerifier::new(vec![signer.public_key()]).expect("configure verifier");
        let domain = CommunityId::from_uuid(Uuid::new_v4());
        let operation_id = Uuid::new_v4();
        let intent_digest = [44; 32];
        let body = lifecycle_body(operation_id, intent_digest, None);
        let url = format!(
            "https://operator.example/operator/communities/{}/identity-lifecycle/retire",
            domain.as_uuid()
        ...
        let first = make_candidate().await.expect("first candidate");
        let second = make_candidate().await.expect("second candidate");
        let replay = ReplaySet::default();
        let (first, second) =
            tokio::join!(first.claim_replay(&replay), second.claim_replay(&replay));
        assert_eq!(usize::from(first.is_ok()) + usize::from(second.is_ok()), 1);
        assert!(
            matches!(first, Err(OperatorLifecycleGrantError::Replayed))
                || matches!(second, Err(OperatorLifecycleGrantError::Replayed))
        );
        assert_eq!(replay.len(), 1);
        assert_eq!(replay.batch_calls(), 2);

        let mut expired = make_candidate().await.expect("candidate to expire");
        expired.grant.expires_at = Utc::now() - chrono::Duration::milliseconds(1);
        let untouched = ReplaySet::default();
        assert!(matches!(
            expired.claim_replay(&untouched).await,
            Err(OperatorLifecycleGrantError::InvalidCredential)
        ));
        assert_eq!(untouched.len(), 0);
        assert_eq!(untouched.batch_calls(), 0);

        let selected = Keys::generate();
        let provision_operation_id = Uuid::new_v4();
        let provision_body = provision_body(domain, provision_operation_id, &selected);
        let provision_url = "https://operator.example/operator/identity-bindings/provision";
        let provision_proof = signed(&signer, provision_url, &provision_body, [1; 32]);
        let make_provision_candidate = || {
            verifier.verify_provision_candidate(
                &provision_proof,
                None,
                provision_url,
                &provision_body,
        ...
        let first = make_provision_candidate()
            .await
            .expect("first provision candidate");
        let second = make_provision_candidate()
            .await
            .expect("second provision candidate");
        let replay = ReplaySet::default();
        let (first, second) =
            tokio::join!(first.claim_replay(&replay), second.claim_replay(&replay));
        assert_eq!(usize::from(first.is_ok()) + usize::from(second.is_ok()), 1);
        ...
        let mut expired = make_provision_candidate()
            .await
            .expect("provision candidate to expire");
        expired.intent.expires_at = Utc::now() - chrono::Duration::milliseconds(1);
        let untouched = ReplaySet::default();
        assert!(matches!(
            expired.claim_replay(&untouched).await,
            Err(OperatorLifecycleGrantError::InvalidCredential)
