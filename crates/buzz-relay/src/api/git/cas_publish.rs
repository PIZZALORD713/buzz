...
pub(crate) struct StagedGitPublication {
...
impl StagedGitPublication {
    ...
    pub(crate) const fn expected_parent_result_digest(&self) -> Option<[u8; 32]> {
...
/// committed `staged.result_digest()` as the canonical PostgreSQL witness and
/// must tolerate [`PointerCacheUpdate::Stale`].
...
pub(crate) async fn publish_pointer_cache(
    ...
    let precond = match store.get_pointer(&pkey).await? {
        ...
        Some((etag, _)) => Precond::IfMatch(etag),
