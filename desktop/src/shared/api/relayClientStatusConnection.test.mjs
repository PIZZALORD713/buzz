...
const RELAY_SIGNER = "dd".repeat(32);
const AUTH_EPOCH = "11111111-1111-4111-8111-111111111111";
      ...
      case "create_auth_event":
        return Promise.resolve(
          JSON.stringify({
            ...
            pubkey: AUTHOR,
            sig: "cc".repeat(64),
            tags: authScopeAvailable
              ? [["buzz_client_binding_scope", "1", AUTH_EPOCH, RELAY_SIGNER]]
              : [],
          }),
        );
...
test("late retirement of connection A cannot clear replacement B", async () => {
  ...
  const currentFor = (author, epoch = AUTH_EPOCH) => ({
  ...
  await connectionA.handleAuthChallenge("challenge-a");
  ...
  await connectionB.handleAuthChallenge("challenge-b");
  const authorB = "ee".repeat(32);
  connectionB.projectionChannel.onmessage(currentFor(authorB, OTHER_EPOCH));
  assert.equal(
    projectionStore.getCurrentProjectionSnapshot(),
    null,
    "projection outside the signed AUTH epoch fails closed",
  );
...
test("ordinary AUTH still succeeds when optional status proof is unavailable", async () => {
  calls.length = 0;
  authDelivery = "normal";
  authScopeAvailable = false;
  socketId = 8_888;
  ...
  assert.deepEqual(
    JSON.parse(authSend.args.message.data)[1].tags,
    [],
    "ordinary NIP-42 remains usable without presentation scope",
  );
  assert.equal(projectionStore.getCurrentProjectionSnapshot(), null);

  client.disconnect();
  authScopeAvailable = true;
});
